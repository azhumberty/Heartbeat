...
public sealed record EnemyDefinition(string Id,string Name,int Health,int Damage,int RewardXp,string Color)
{
    public static readonly EnemyDefinition[] All={
        new("forest","Guardião de raízes",72,14,24,"64856b"),
        new("night","Predador do breu",96,18,38,"725584"),
        new("ruin","Vigia esquecido",115,17,48,"a58c65"),
        new("camp","Saqueador da fogueira",84,16,30,"ac684d")};
...
public sealed class CombatManager
{
    ...
    public static CombatManager Start(GameSave game,Dictionary<string,CardDefinition> catalog,string encounter,string enemy,string arena,float hour)
    {
        ...
        string problem=DeckManager.Validate(game.Deck,catalog);if(problem.Length>0)throw new ArgumentException(problem);
        var foe=EnemyDefinition.Get(enemy);
        var state=new CombatState {EncounterId=encounter,EnemyId=enemy,Arena=arena,Hour=hour,Seed=Random.Shared.Next(),
            Player=new(){Health=Math.Max(1,game.Player.Health),MaxHealth=game.Player.MaxHealth},
            Enemy=new(){Health=foe.Health,MaxHealth=foe.Health},Mana=game.Player.Mana,MaxMana=game.Player.MaxMana,
            ReturnX=game.PlayerX,ReturnZ=game.PlayerZ,ReturnYaw=game.PlayerRotationY,ClockWasPaused=game.Settings.WorldTimePaused};
        ...
        game.ActiveCombat=state;var manager=new CombatManager(game,state);manager.Shuffle();manager.BeginTurn();
    ...
    {
        string error=CanPlay(index);if(error.Length>0)return error;
        string id=State.Hand[index];var c=State.Cards[id];State.Mana-=c.Cost;
    ...
    public void EndTurn()
    {
        ...
        // Preserve a companion until played; ordinary hand is discarded.
        var retained=State.Hand.Where(id=>State.Cards[id].Category==CardCategory.Companion).ToList();
        State.Discard.AddRange(State.Hand.Where(id=>State.Cards[id].Category!=CardCategory.Companion)); State.Hand=retained;
        Tick(State.Enemy);Resolve();if(State.Result.Length>0)return;
        ...
        Decay(State.Enemy);
        Resolve();if(State.Result.Length==0)BeginTurn();
    }
    void BeginTurn()
    {
        State.Turn++;State.Player.Shield=0;
        Tick(State.Player);Resolve();if(State.Result.Length>0)return;
    ...
    public void Draw(int count)
    {
        for(int i=0;i<Math.Clamp(count,0,10)&&State.Hand.Count<10;i++)
        {
            ...
            string id=State.DrawPile[^1];State.DrawPile.RemoveAt(State.DrawPile.Count-1);State.Hand.Add(id);
        }
    }
    void Shuffle()
    {
        var rng=new Random(unchecked(State.Seed+State.ShuffleCount++*7919));
        for(int i=State.DrawPile.Count-1;i>0;i--){int j=rng.Next(i+1);(State.DrawPile[i],State.DrawPile[j])=(State.DrawPile[j],State.DrawPile[i]);}
    }
    void Apply(CardEffect e)
    {
        ...
        switch(e.Kind)
        {
            ...
            default:target.Status[e.Kind]=Math.Min(3,Math.Max(target.Status.GetValueOrDefault(e.Kind),e.Duration));target.Potency[e.Kind]=Math.Clamp(e.Value,1,4);break;
        }
    }
    static void Hurt(CombatantState target,int amount,CombatantState source)
    {
        if(source.Status.GetValueOrDefault(EffectKind.Strengthened)>0)amount+=source.Potency.GetValueOrDefault(EffectKind.Strengthened,3);
        if(target.Status.GetValueOrDefault(EffectKind.Vulnerable)>0)amount=(int)Math.Ceiling(amount*1.5);
        int absorbed=Math.Min(target.Shield,amount);target.Shield-=absorbed;target.Health=Math.Max(0,target.Health-amount+absorbed);
    }
    static void Tick(CombatantState target){if(target.Status.GetValueOrDefault(EffectKind.Bleeding)>0)target.Health=Math.Max(0,target.Health-target.Potency.GetValueOrDefault(EffectKind.Bleeding,3));}
    static void Decay(CombatantState target){foreach(var key in target.Status.Keys.ToArray()){target.Status[key]=Math.Max(0,target.Status[key]-1);}}
    void Resolve(){if(State.Enemy.Health<=0)State.Result="Victory";else if(State.Player.Health<=0)State.Result="Defeat";}
    ...
    public void Settle()
    {
        ...
        {
            p.Victories++;p.Resolved=!State.Repeat;
            int xp=State.RewardXp;_game.Player.Experience+=xp;
            _game.Player.Level=1+_game.Player.Experience/100;
            var generic=new CardRepository().Generic;
            State.RewardId=generic[rng.Next(generic.Count)].Id;
            _game.Deck.Owned[State.RewardId]=_game.Deck.Owned.GetValueOrDefault(State.RewardId)+1;
            int coins = rng.Next(15, 30);
            _game.Player.Coins += coins;
            State.RewardText=$"+{xp} XP · +{coins} Reais · Carta: {generic.First(c=>c.Id==State.RewardId).Name}";
            _game.Player.Health=State.Player.Health;
        }
        else if (State.IsDuel)
        {
            // Duelo amistoso: sem game over, dá algum prêmio de consolação
        ...
        {
            _game.Player.Health=State.Result=="Defeat"?Math.Max(1,_game.Player.MaxHealth/2):Math.Max(1,State.Player.Health-5);
            if(State.Result=="Defeat"){State.ReturnX=0;State.ReturnZ=5;_game.Player.Experience=Math.Max(0,_game.Player.Experience-5);}
            State.RewardText=State.Result=="Defeat"?"Você desperta perto da cabana · metade da Vida · −5 experiência":"Retirada · −5 Vida · território em alerta";
        }
        _game.Player.Mana=Math.Min(_game.Player.MaxMana,Math.Max(20,State.Mana));
        _game.Player.Clamp();_game.Encounters[State.EncounterId]=p;
        ...
        _game.PlayerX=State.ReturnX;_game.PlayerZ=State.ReturnZ;_game.PlayerRotationY=State.ReturnYaw;
    }
}
