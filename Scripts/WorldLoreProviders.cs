...
{
    Task<WorldLoreGeneration> CreateAsync(long seed,GameSettings settings,CancellationToken token=default);
}
public sealed class OfflineWorldLoreProvider : IWorldLoreProvider
{
    public Task<WorldLoreGeneration> CreateAsync(long seed,GameSettings settings,CancellationToken token=default){token.ThrowIfCancellationRequested();return Task.FromResult(new WorldLoreGeneration{Lore=WorldLoreManager.CreateOffline(seed)});}
}
public sealed class GroqWorldLoreProvider : IWorldLoreProvider
{
    ...
    public async Task<WorldLoreGeneration> CreateAsync(long seed,GameSettings settings,CancellationToken token=default)
    {
        if(string.IsNullOrWhiteSpace(System.Environment.GetEnvironmentVariable("GROQ_API_KEY")))return await Fallback(seed,settings,"IA indisponível · introdução criada offline",token);
        try
        {
            if(!Uri.TryCreate(settings.Endpoint,UriKind.Absolute,out var endpoint)||endpoint.Scheme!="https"||endpoint.Host!="api.groq.com")return await Fallback(seed,settings,"Endpoint inválido · introdução criada offline",token);
            ...
            var prompt="Crie a introdução original de uma campanha 2D medieval dark, romântica e misteriosa em português. O jogador ainda escolherá seu nome. Use apenas inspiração nos assets, sem citar IDs. Responda somente no JSON do schema. Contexto: "+JsonSerializer.Serialize(new{seed,assets,cast});
            bool gptOss=settings.Model.StartsWith("openai/gpt-oss",StringComparison.OrdinalIgnoreCase);
            var request=new Dictionary<string,object>{["model"]=settings.Model,["temperature"]=Math.Clamp(settings.Temperature,0f,1.1f),["max_completion_tokens"]=gptOss?900:500,["response_format"]=gptOss?StrictFormat():new{type="json_object"},["messages"]=new[]{new{role="user",content=prompt}}};
        ...
        catch(Exception e){GD.Print($"[AI] Introdução Groq indisponível ({e.GetType().Name}); usando offline");return await Fallback(seed,settings,"Falha na IA · introdução criada offline",token);}
    }
    async Task<WorldLoreGeneration> Fallback(long seed,GameSettings settings,string status,CancellationToken token){var generated=await _fallback.CreateAsync(seed,settings,token);return new WorldLoreGeneration{Lore=generated.Lore,ProviderStatus=status};}
