...
public sealed class GroqEventProvider : IEventProvider
{
    ...
    static string BuildPrompt(EventContext context)
    {
        ...
        var compact=new {player=context.Game.PlayerName,seed=context.Game.WorldSeed,region=context.Game.WorldLore.RegionName,premise=context.Game.WorldLore.Premise,node=new {context.Node.Id,context.Node.Title,kind=context.Node.Kind.ToString(),context.Node.Description},assets,recent};
